function [virtualMachinesInServer, valueVMs] = createNewVirtualMachinesGreedyOneFuc(tasksInterval, taskIndex, serverIndex, vCPUs_server, GPUs_server, periodTasks, totalLoadCpus, ...
    loadCPU, loadGPU, alpha, lamdaMatrix, chiMatrix, powerMatrix, energyInServer, virtualMachinesInServer, valueVMs)

valueServerJ = 0;
vCPUs = vCPUs_server(serverIndex);
num_gpu = GPUs_server(serverIndex);
gpu_valid = ones(1, num_gpu);
